...
Таблица выше — шортлист. Полный снимок из 343 моделей с ценами, возможностями и
индексами качества: [`docs/research/2026-09-23-openrouter-catalog.md`](research/2026-09-23-openrouter-catalog.md)
и `data/openrouter-models.json`.

Главный вывод оттуда: **разрыв в качестве между ступенями около 20%, разрыв в
цене — от 67 до 250 раз.** `glm-5.3-flash` за $0.15/$0.50 даёт агентность 50.9
против 51.0 у `gpt-6-astra` за $10/$50.

Скрипт перезапускается одной командой — каталог меняется каждую неделю:

```bash
uv run python scripts/model_catalog.py --sort value --has tools --json data/openrouter-models.json
```
