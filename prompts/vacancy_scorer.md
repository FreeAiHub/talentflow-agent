...
> **Зачем отдельный промпт.** Ни один из остальных промптов не решает задачу
> «стоит ли вообще брать эту вакансию в работу»: `response_generator.md` пишет
> письмо, `grounding_checker.md` проверяет уже написанное. Оценка вакансии как
> лида — отдельный шаг перед ними.
>
> Историческая справка: `vacancy_analyzer.md`, `archetype_matcher.md` и
> `quality_scorer.md` остались от прежней сервисной архитектуры и кодом не
> загружаются.
